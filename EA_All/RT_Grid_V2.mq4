...
//|In Progress: MA Entry, Reverse MA Entry
//+------------------------------------------------------------------+
...
input bool             _gapCloseCheck1=true;             //Use Close Order After certain Time
...
extern string          MA_Set="--MA Settings--";           //MA:: Settings
input ENUM_TIMEFRAMES  MA_Chart_Timeframe=PERIOD_CURRENT;  //MA:: Chart Time 
input int              MA_Period=14;                       //MA:: Period
input int              MA_Shift=0;                         //MA::Shift
input ENUM_MA_METHOD   MA_Method;                          //MA::Method
input ENUM_APPLIED_PRICE MA_ApPrice=PRICE_CLOSE;           //MA::Applied Price
...
int Pattern_MA()
{
   ...
   if(Volume[0]>1) return FAIL;
//--- get Moving Average 
   double ma=iMA(NULL,0,MA_Period,MA_Shift,MA_Method,MA_ApPrice,0);
//--- buy conditions
                        ...
                        if(OrderStopLoss()>OrderOpenPrice())
                          {
                           ...
                           && OrderStopLoss()<OrderOpenPrice())
                             {
                              //double trailStop=trail-trail*0.25;
                              double trailStop =min_stop;
                              //if(NormalizeDouble(Bid - trailStop*point,digit)> NormalizeDouble(min_stop*point,digit))
                              //{
                                 double stop=NormalizeDouble(Bid-trailStop *point,digit);
                                 if(stop- Bid > min_stop )
                                 {
                                    Print("Trail by points[",trailStop,"]");
                                    Trail(stop);
                                 }
                              ...
                              OrderStopLoss()>OrderOpenPrice()) || OrderStopLoss()==0)
                                {
                                 ...
                                 double trailStop= min_stop;
                                 double stop=NormalizeDouble(Ask+trailStop *point,(int)MarketInfo(Symbol(),MODE_DIGITS));
                                 if(Ask-stop>NormalizeDouble(min_stop*point,digit))
                                 {
                                    Print("Trail by points[",trailStop,"]");
                                    Trail(stop);
                                 }
                  ...
                  if(StringFind(OrderComment(),comment,0)!=-1)
                    {
                     if(OrderType()==OP_BUY)
                     ...
                     else if(OrderType()==OP_SELL)
                       {
                        if(OrderProfit()>=_profitTarget)
                          {
                           //OrderClose(OrderTicket(),OrderLots(),Ask,Slippage,clrAntiqueWhite);
                           CloseAllOrders();
                          }
                        if(OrderProfit()<=_stopLevel && useGridStop==true)
                          {
         ...
         if(order1Open)
           {
            ...
            //Trailing_Stop(_trail1,Strat_Name1,_trailPoint1);
            ...
            if(useStrategy1)
              {
               ...
               {
                  if(Pattern_MA()==BUY && checkSell==false)
                    {
                     Buy_Order(startLot,Strat_Name1,SL_Type1,SL_Fixed1,SL_Volatility_Factor1,
                               TP_Type1,TP_Fixed1,TP_Volatility_Factor1,OP_BUY);
                     prevLotBuy=startLot;
                  ...
                  else if(Pattern_MA()==SELL && checkBuy==false)
                    {
                     Sell_Order(startLot,Strat_Name1,SL_Type1,SL_Fixed1,SL_Volatility_Factor1,
                                TP_Type1,TP_Fixed1,TP_Volatility_Factor1,OP_SELL);
                     prevLotSell= startLot;
