//+------------------------------------------------------------------+
//|                                                   Swing Band.mq4 |
//|                                      Copyright 2016, Awais Tariq |
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright   "Copyright 2016, Awais Tariq"
#property description " Awais Tariq"
...
input bool             newbb=true;   //Use New BB?
input int              bbper=20;     //New BB Period
input int              bbdev=3;      //New BB Deviation
input bool             rsibool=true; //Use RSI?
input int              rsiper=14;    //RSI Period
input int              rsih=70;      //RSI High level
input int              rsil=30;      //RSI Low level
input bool             macdbool=true;//Use MACD?
input double           macdlvlh=0;   //MACD level Sell
input double           macdlvll=0;   //MACD level Buy
input int              macd1=12;     //MACD fast ema
input int              macd2=26;     //MACD slow ema
input int              macd3=9;      //MACD signal
...
//---New BB
     ...
     {
      double bbh=iBands(_Symbol,PERIOD_CURRENT,bbper,bbdev,0,PRICE_CLOSE,MODE_UPPER,1);
      double lastbbh=iBands(_Symbol,PERIOD_CURRENT,bbper,bbdev,0,PRICE_CLOSE,MODE_UPPER,2);
      double rsi=iRSI(_Symbol,PERIOD_CURRENT,rsiper,PRICE_CLOSE,1);
      double macd=iMACD(_Symbol,PERIOD_CURRENT,macd1,macd2,macd3,PRICE_CLOSE,MODE_SIGNAL,1);

      if(Close[1]>=bbh && Close[2]<lastbbh)
        {
         Print("UP Band Signal Activated");
         bbhinit=true;
        }
      ...
      if(Close[1]>=bbh && bbhinit && (!rsibool || (rsibool && rsi>=rsih)) && (!macdbool || (macdbool && macd>=macdlvlh)))
        {
         Print("Up band SELL");
         bbhinit=false;
         return true;
...
//---New BB
     ...
     {
      double bbl=iBands(_Symbol,PERIOD_CURRENT,bbper,bbdev,0,PRICE_CLOSE,MODE_LOWER,1);
      double lastbbl=iBands(_Symbol,PERIOD_CURRENT,bbper,bbdev,0,PRICE_CLOSE,MODE_LOWER,2);
      double rsi=iRSI(_Symbol,PERIOD_CURRENT,rsiper,PRICE_CLOSE,1);
      double macd=iMACD(_Symbol,PERIOD_CURRENT,macd1,macd2,macd3,PRICE_CLOSE,MODE_SIGNAL,1);

      if(Close[1]<=bbl && Close[2]>lastbbl)
        {
         Print("LOW Band Signal Activated");
         bbinit=true;
        }
      ...
      if(Close[1]<=bbl && bbinit && (!rsibool || (rsibool && rsi<=rsil)) && (!macdbool || (macdbool && macd<=macdlvll)))
        {
         Print("Low band BUY");
         bbinit=false;
         return true;
         ...
         if(Buy_Alert())
           {
            ...
            if(ignore<1)
              ...
              {
               s_loss--;Cs_loss--;ignore--; Print("Short Candle Ignored");
               Print("Next ",ignore," short Candles will be  ignored");
              }
           }
         else if(Sell_Alert())
