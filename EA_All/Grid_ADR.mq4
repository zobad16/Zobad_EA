  ...
  /* Check_Market();
   ...
   if(order1Open)
   {
      ...
      {            
         ...
         if(entry_type== ADR)
         {
            int pattern = Pattern_Point();
            if(pattern==BUY && checkSell==false && buy_counter<numberOfLegs)
            {
            ...
            else if(pattern==SELL && checkBuy==false && sell_counter<numberOfLegs)
            {
         ...
         if(order1Open)
           ...
           if(!(tick==FAIL))
            ...
            //int ticket, int sl_type, double _slf, double _slv ,int tp_type, double _tpf, double _tpv)
              _p_OrderModify(tick,SL_Type2,SL_Fixed2,SL_Volatility_Factor2,
                               TP_Type2,TP_Fixed2,TP_Volatility_Factor2);
               //PendingModify(SL_Type2,SL_Fixed2,SL_Volatility_Factor2,
                //               TP_Type2,TP_Fixed2,TP_Volatility_Factor2);
               //OrderModify(tick,);
            }
            ...
            if(IsNewBar())
              {
               ...
               if(entry_type == BREAK && useGridding ==true)
               {
                ...
                /*else if(entry_type== ADR && useGridding ==true)
                {
                   if(Pattern_Point()==BUY && checkSell==false && buy_counter<numberOfLegs)
                    {
                     Print("Buy Counter [",buy_counter);
