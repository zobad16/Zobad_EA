            ...
            if(algo.OrderOperationCode(Magic_Number) == BUY_LEG1)
            {
               ...
               else if(useGHedge==true && (algo.Pattern_Point_Negative(pointsE) == REVERSAL_BUY)&& bcount==numberOfLegs)
               {
                  ...
                  /*double pre_lot,n_lot= 0.0;
                     ...
                     if(algo.isExist(Magic_Number,(string)SELL_LEG2,count,op_c,pre_lot))
                     {n_lot = mm.CalculatePositionSizeHedge(Magic_Number,SELL_LEG2);}
                     else if(algo.isExist(Magic_Number,(string)SELL_LEG2,count,op_c,pre_lot)==false){
                        n_lot = mm.CalculatePositionSizeHedge(Magic_Number,SELL_LEG1);//Accumalative lot= prev_lot*increaseLLotBy;
                     }
                     prev_lot = n_lot;
                     ccode    = HEDGE_SELL;
                     op_code  = DIRECTIONAL_BUY;
...
//strat = Directional/Reversal Buy/Sell
/*
bool Revised_Buy(int strat,int rt_Code)
{   
   double tp =0.0, sl =0.0;
   double lot = mm.CalculatePositionSize(lotType,LotSize,_risk);
   if     (rt_Code == (string)DIRECTIONAL_BUY)
   {       
      mm.PlaceOrder(OP_BUY,lot,TP_Type1,TP_Value1,SL_Type1,SL_Value1,Magic_Number,rt_Code);
   }        
   else if(rt_Code == (string)REVERSAL_BUY)
   {        
      mm.PlaceOrder(OP_BUY,lot,TP_Type,TP_Value,SL_Type,SL_Value,Magic_Number,rt_Code);   
   }         
   return false;
}*/
