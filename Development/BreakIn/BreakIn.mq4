...
extern string          Strat_Name1v2="Break In2";         //Strategy Name
...
input bool             _use_risk_candle2=false;             //Use Risk Management
input int              _risk_candle2=4;                    //candles to Read for Risk Management
double                 _highestStop2;
double                 _lowestStop2;
input bool             _gapCloseCheck2=false;               //Use Close Candle after a certain time gap
...
input bool             _use_risk_candle3=false;             //Use Risk Management
input int              _risk_candle3=4;                    //candles to Read for Risk Management
double                 _highestStop3;
double                 _lowestStop3;
input bool             _gapCloseCheck3=false;               //Use Close Candle after a certain time gap
...
input bool             _use_risk_candle4=false;            //Use Risk Management
input int              _risk_candle4=4;                  //candles to Read for Risk Management
double                 _highestStop4;
double                 _lowestStop4;
input bool             _gapCloseCheck4=false;              //Use Close Candle after a certain time gap
...
input double           adx_Max_Range=30;               //ADX::Max Range
input double           adx_Min_Range=15;               //ADX::Minimum Range
...
bool Place_Buy_Order4()
{
  ...
  if(Ask-sl>=minSL)
   ...
   if(Ask-sl<minSL)
   {  
      double toAdd= minSL +(10.0 * _point);      
      sl= Ask- toAdd;      
...
bool isADX()
{
   if(use_ADX == false){return true;}
   int digit=(int)MarketInfo(Symbol(),MODE_DIGITS);
   double adx = NormalizeDouble(iADX(Symbol(),adx_Chart_Timeframe,adx_Period,adx_App_Price,0,0),digit);
   if(adx > adx_Min_Range && adx< adx_Max_Range)
   {
      Print("ADX[",adx,"]");
      return true;
   }
...
//---
   ...
   if(order1Open)
    ...
    if(order1v2Open)
      {
         Trailing_Stop(_trail1v2,Strat_Name1v2, _trailPoint1v2);
   ...
   if(IsNewBar() && isADX())
      {
        ...
        if(useStrategy1v2)
        {
         ...
         if(Pattern1v2()==SELL && !sellIgnore1v2==true)
            {
