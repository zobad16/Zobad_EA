...
extern string           pairX              = "US30.CASH"    ;             //LeadPair
extern string           pairY              = "US500.CASH"   ;             //FollowerPair1
extern string           pairZ              = "USNDX.CASH"   ;             //FollowerPair2
bool             visible            = false      ;
...
void OnTick()
  {
//---
   Comment("Lead: "+pairX+"|Follower1:"+pairY+"|Follower2"+pairZ+" tp["+(string)_tp+"] sl["+(string)_sl+"]");
   int profitOrLoss = 0;
   if(isOrdersTotal(Magic_Number)==0){   
      ...
      if(_tp > 0 && _sl>0){
            if(op == OP_BUY){
                           if(_Ask >= _tp){mm.CloseAllOrders(Magic_Number);/*Close All Orders*/}
                           
                           if(isGridTrigger(gridDirection,Magic_Number,pairX,stThresh,gridMultiply,prevThresholdLvl)){
                              PlaceOrderPair(OP_BUY,OP_SELL,OP_SELL,"Spread Buy");Print("Spread Buy");
                           }
            }
            else if(op == OP_SELL){
                           if(_Ask >= _sl){mm.CloseAllOrders(Magic_Number);/*Close All Orders*/}
                           else if(_Bid<= _tp){mm.CloseAllOrders(Magic_Number);/*Close All Orders*/}
            } 
      }  
      mm.EquityBasedClose(EQ_Based,_profitTarget,useGridStop,_stopLevel,Magic_Number,profitOrLoss);
   }
...
double calculateEquityGrid(double startThreshold, double mult, double prevleg){
   double threshold = (prevleg + startThreshold)*mult;
   return threshold;
}
...
bool isGridTrigger(int direction,int magic, string symbol, double stThresh, double mult, double prevleg){
   ...
   double threshold = calculateEquityGrid(stThresh,mult,prevleg);
   Print("legCount[",IntegerToString(legCount),"],threshold[",DoubleToString(threshold),"]");
   //if positive grid:
   if(direction == PositiveGrid){  return(getEquity(magic)>threshold?true:false);}
   else if(direction == NegativeGrid) return(getEquity(magic)<threshold?true:false);
