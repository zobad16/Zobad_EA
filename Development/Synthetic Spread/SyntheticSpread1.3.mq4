...
//---------------------------------------------------------------------
//| Updates: Added SpreadHedge
//|------------------------------------------------------------------|
//| New: Seperate grid entry based on equity from equity based close(pending)
//+------------------------------------------------------------------+
...
extern bool             useSpreadHedge     = false     ;             //Strategy:: Use Spread Hegde 
...
extern GridDirection   gridDirection      = NegativeGrid;         //Grid::Direction
extern double          gridEntry          = -100;                 //Grid::Entry$
extern int             gridMultiply       = 2;                    //Grid::Entry threshold Multiply
extern int             gridLeg            = 3;                    //Grid::Legs
extern bool            EQ_Based           = true      ;           //Equity Stop::Use Equity Based TP
extern bool            useGridStop        = true     ;             //Equity Stop::Use Equity Based SL
extern double          _profitTarget      = 1000      ;             //Equity Stop::Profit Target
extern double          _stopLevel         = -500.0    ;             //Equity Stop::Stop Out Level
   ...
   if(isOrdersTotal(Magic_Number)==0){   
      ...
      if(_tp > 0 && _sl>0){
            if(op == OP_BUY){
                           ...
                           //Hedge Spread
                           if(useSpreadHedge){
                              PlaceOrderPair(OP_SELL,OP_BUY,OP_BUY,"Spread Buy-Hedge");
                              Print("Spread Buy-Hedge");
                           }
            }
            else if(op == OP_SELL){
                           ...
                           //Spread Hedge
                           if(useSpreadHedge){
                              PlaceOrderPair(OP_BUY,OP_SELL,OP_SELL,"Spread Sell-Hedge");
                              Print("Spread Sell-Hedge");
                           }
            } 
      }  
