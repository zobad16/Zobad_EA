...
extern string          Strat_Name1v2="Break Out2";         //Strategy Name
input bool             useStrategy1v2=true;                //Use Strategy
         ...
         if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)==true)
          {
            if(OrderMagicNumber() == Magic_Number && OrderSymbol() == Symbol())
            {
               if(StringFind(OrderComment(), comment,0)!=-1)
               {
                 ...
                 double dif= Time[0]-OrderOpenTime() ;
...
//---
   ...
   if(order5Open)
   {
      /*Trailing_Stop(_trail5, Strat_Name5, _trailPoint5);
      JumpToBreakeven(_breakEven5,Strat_Name5,_whenJump5,_jumpBy5);
      if(_use_risk_candle5)MarketClose(Strat_Name5,_lowestStop5, _highestStop5);
      riskManagementClose(_gapCloseCheck5,Strat_Name5,_whenClose5);     */
      if(Pattern5()==BUY)
      {
         Buy_Order();//Parameters need to think
      }
   }
   if(IsNewBar())
      {
       ...
       if(useStrategy3 )
        ...
        if(useStrategy5)
        {
           Order_Ignore(Strat_Name5,_timegap5, buyIgnore5, OP_BUY);
           Order_Ignore(Strat_Name5,_timegap5, sellIgnore5, OP_SELL);
           if(Pattern5()==BUY && buyIgnore5 == false)
           {
            Buy_Order(initial_lotsize,Strat_Name5,SL_Type5, 0.0,0.0,
                      TP_Type5, TP_Fixed5, TP_Volatility_Factor5, OP_BUY);
           }
           else if(Pattern5()==SELL && sellIgnore5 ==false)
           {
            Sell_Order(initial_losize,Strat_Name5,SL_Type5,0.0,0.0,
                       TP_Type5,TP_Fixed5,TP_Volatility_Factor5,OP_SELl);
           }
        } 
      }
  }
