...
//---
   ...
   Dialog.Create(ChartID(),"                                      ALGOTRADEUP",0,5,5,400,350);
   ...
   MannualEntryBtn[1].Create(0,"Open_Now",0,6+110, 210,0,0)             ;
   MannualEntryBtn[1].Text("Open Now")                            ;
...
//---
   ...
   {
      if(!_flag){
         if(ask == Settlement_Price|| bid == Settlement_Price )
            PlacePendingOrders();         
      }   
...
bool ModifyTP(){
   ...
   if(count == 1 && count_p >0){
      Alert("Count: "+count_p+" |Ticket: "+ticket_p);
