...
CisNewBar cchart; // instance of the CisNewBar class: current chart
CDealInfo m_deal;
CDialog Dialog;
...
enum E_SST
{
   ...
   MA_Reversal = 3,
   Open_Now=4,
   MA_Breakout_Directional =5,
   MA_Breakout_Reversal=6
};
enum Stop_Types
{
   None=0,
...
input double  TP_Point=12000;//TP Point
input double  TP_Money=240;//TP $Money
input Stop_Types Sl_Type = Fix;
input double  SL_Value = 4000;
input double  Grid_Risk_Money=80;//Grid Risk $Money
...
input bool    UseOppositeOrder = false; //Use Opposite Order Filter
input bool    UseTradeDelay  = false; //Use Trade Dealy
input int     TradeDelay     = 15; //Trade Delay(mins)
input bool  UseAtrFilter        =false;  //Use ATR Filter
input double  ATR_Threshold  = 100.00; //ATR Threshold
...
input bool    UseDistanceFilter = false;//Use Distance filter
input int      Min_Distance = 10; //Minimum distance
input int      Max_Distance = 20; //Max distance 
...
input bool DevLogs = false;   //DevLogs- keep it false
//input 
double TPB,TPS;
double SLB,SLS;
int C_B=-1;
...
{
   //if(!UseTradeDelay)return;
   if(!UseOppositeOrder)return; 
   ...
   if(type == TRADE_TRANSACTION_DEAL_ADD){
      ...
      if((ENUM_DEAL_REASON)reason==DEAL_REASON_SL||(ENUM_DEAL_REASON)reason==DEAL_REASON_TP
          || (ENUM_DEAL_REASON)reason==DEAL_REASON_CLIENT || (ENUM_DEAL_REASON)reason==DEAL_REASON_MOBILE|| (ENUM_DEAL_REASON)reason==DEAL_REASON_EXPERT)
          {
      
         if(m_deal.DealType() == DEAL_TYPE_BUY || m_deal.DealType()== DEAL_TYPE_SELL ){
            if(m_deal.Entry() == DEAL_ENTRY_OUT){
               Print("Position closed. Reason: "+EnumToString((ENUM_DEAL_REASON)reason));
               long position_id = m_deal.PositionId();
               
               long mg = m_deal.Magic();
               
               if(HistorySelectByPosition(position_id)){
                  string comm = PositionGetString(POSITION_COMMENT);
                  long mgg= PositionGetInteger(POSITION_MAGIC);      
                  Print("Position Magic_Number: "+IntegerToString(mgg)+", position id: "+IntegerToString(position_id)+", Comment: "+comm);
                  if(mgg == magic_num && PositionGetString(POSITION_SYMBOL) == Symbol()){
                     Alert("Closed position Ticket no: ",position_id);
                     ...
                     if(dir == POSITION_TYPE_BUY) {
                       ...
                       Print("isBUY: "+(string)isBuy+" IsSell: "+(string)isSell);
                     }    
               }               
            }
         }
...
//---
   ...
   if(cchart.isNewBar(new_time)) OnNewBar();               // When new bar appears - launch the NewBar event handler
   ...
   if(Sl_Type == ATR){
      double   atr[1];
         ...
         SLS=bid+SL_Value*Point();
         SLB=ask-SL_Value*Point();
   ...
   if((Strategy_Type==Open_Now )&& (Grid_Direction==0||Grid_Direction==2) && CalculateCurrentOrders(0)==0)
   {
   ...
   else if((Strategy_Type==Open_Now  )&& (Grid_Direction==1||Grid_Direction==2) && CalculateCurrentOrders(1)==0)
    {
     ...
     //MA Entry
     if(orders_total==0){
      if((ma_signal== 1 && (Strategy_Type == MA_Directional || Strategy_Type == MA_Breakout_Directional || Strategy_Type == MA_Reversal || Strategy_Type == MA_Breakout_Reversal )))
      {
         if(Check_Distance(UseDistanceFilter,1,Max_Distance,Min_Distance)){
            Refresh_Sell();
            Refresh_Buy();
            if(trade.Buy(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_ASK),Sl_Type==None?NULL:SLB,Tp_Type==None?NULL:TPB,Comment_Order)){
               C_B=0;
               GB[C_B]=SymbolInfoDouble(Symbol(),SYMBOL_ASK);
               GBL[C_B]=Lot_Size;
               no_new_trade = true;
      ...
      else if((ma_signal== -1 && (Strategy_Type == MA_Directional|| Strategy_Type == MA_Breakout_Directional || Strategy_Type == MA_Reversal || Strategy_Type == MA_Breakout_Reversal)))
       {
         if(Check_Distance(UseDistanceFilter,1,Max_Distance,Min_Distance)){
            Refresh_Sell();
            Refresh_Buy();
            if(trade.Sell(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_BID),Sl_Type==None?NULL:SLS,Tp_Type==None?NULL:TPS,Comment_Order)){
               C_S=0;
               GS[C_S]=SymbolInfoDouble(Symbol(),SYMBOL_BID);
               GSL[C_S]=Lot_Size;
               no_new_trade = true;
   ...
   if( orders_total==0 /*&& iVolume(NULL,PERIOD_CURRENT,0)<=1*/)
   {
      ...
      if(UseTradeDelay && (current_time+(TradeDelay*60)) <= TimeCurrent() ){
         Alert(TradeDelay," min have passed. Turning on trades");
...
void OnNewBar(){
   if(!no_new_trade)return;
   int orders = CalculateCurrentOrders2();
   if(orders == 0 ){
      Print("New bar detected, reactivating orders signal check");
      no_new_trade = false;
   }   
   //if orders == 0
   //if no_new_trade == true
   //set no_new_trade =false
}
//+------------------------------------------------------------------+
//| ChartEvent function                                              |
//+------------------------------------------------------------------+
...
bool LegEntryCheck(){
   ...
   if(orders_buy > 0){
      ...
      if(Bid <= price - distance ){
         Print("Buy Legs["+IntegerToString(orders_buy)+"] Sell Legs["+IntegerToString(orders_sell)+"]");
   ...
   if(no_new_trade){
    ...
    if(Strategy_Type==MA_Directional || Strategy_Type == MA_Reversal){
      ...
      if(!SymbolInfoTick(_Symbol,tick)) { Print("no tick data available, error = ",GetLastError()); ExpertRemove(); return -99; }
       ...
       //DIRECTIONAL
      if((close >= ma[0] && close1 < ma[0]) &&
          Strategy_Type==MA_Directional  ){
            signal = 1;
            if(DevLogs)Print(Symbol()+": Buy signal- Directional");
       }     
      else if((close <= ma[0] && close1 > ma[0])&&
          Strategy_Type==MA_Directional) {
            signal = -1;
            if(DevLogs)Print(Symbol()+": Sell signal- Directional");
      }
      //Reversal
      if((Ask >= ma[0] && open < ma[0]) &&
          Strategy_Type==MA_Reversal  ){
            signal = -1;
            if(DevLogs)Print(Symbol()+": Sell signal- Reversal");
          }
          ...
          {
            signal = 1;
            if(DevLogs)Print(Symbol()+": Buy signal- Reversal");
          }        
      if(UseOppositeOrder){
         if(signal == 1 && isBuy){
            signal = 0;
            if(DevLogs)Print(Symbol()+"Buy signal ignored. Reason UseOpposite Filter on and previous order buy");
         }   
         else if(signal == -1 && isSell){
            signal=0;
            if(DevLogs)Print(Symbol()+"Sell signal ignored. Reason UseOpposite Filter on and previous order sell");
            }
      }
      return signal;   
    }
   else if(Strategy_Type==MA_Breakout_Directional || Strategy_Type == MA_Breakout_Reversal){
      ...
      if((open1 <= ma[1] && close1 >= ma[1]) &&
          Strategy_Type==MA_Breakout_Directional  ){
            signal = 1;
            if(DevLogs)Print(Symbol()+": Buy signal- Breakout directional");
       }     
      else if((open1 >= ma[1] && close1 <= ma[1])&&
          Strategy_Type==MA_Breakout_Directional){ 
            signal = -1;
            if(DevLogs)Print(Symbol()+": Sell signal- Breakout directional");
      }
      //Reversal
      if((open1 <= ma[1] && close1 >= ma[1]) &&
          Strategy_Type==MA_Breakout_Reversal  ){
            signal = -1;
            if(DevLogs)Print(Symbol()+": Buy signal- Breakout reversal");
      }      
      else if((open1 >= ma[1] && close1 <= ma[1])&&
          Strategy_Type==MA_Breakout_Reversal){ 
            signal = 1;
            if(DevLogs)Print(Symbol()+": Sell signal- Breakout reversal");
      }          
      if(UseOppositeOrder){
         if(signal == 1 && isBuy){
            signal = 0;
...
void MannualEntry(int op)
{
   ...
   if(op == ORDER_TYPE_BUY){
      if(CalculateCurrentOrders(ORDER_TYPE_BUY)==0 && (Grid_Direction==Grid_Long||Grid_Direction==Grid_Both)){
         Refresh_Sell();
         CalculateStops(1,tp,sl);
         if(trade.Buy(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_ASK),sl,tp,Comment_Order)){
            C_B=0;
            GB[C_B]=SymbolInfoDouble(Symbol(),SYMBOL_ASK);
            GBL[C_B]=Lot_Size;
            no_new_trade = true;
      ...
      if(CalculateCurrentOrders(ORDER_TYPE_SELL)==0)
      {
         Refresh_Buy();
         CalculateStops(-1,tp,sl);
         if(trade.Sell(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_BID),sl,tp,Comment_Order)){
            C_S=0;
            GS[C_S]=SymbolInfoDouble(Symbol(),SYMBOL_BID);
            GSL[C_S]=Lot_Size;
            no_new_trade = true;         
...
bool IsNewBar(){
  int candle = Bars(_Symbol, _Period);
...
bool Check_Distance(bool use, int directional, double max, double min){
   if(!use)return true;
   double   ma[2];
   if(CopyBuffer(ExtHandle,0,0,2,ma)!=2)
     {
   ...
   if(!SymbolInfoTick(_Symbol,tick)) { Print("no tick data available, error = ",GetLastError()); ExpertRemove(); return false; }
    double Ask=tick.ask;
    double Bid=tick.bid;
    if(Ask > ma[0] && directional ==1){
      double distance = Ask- ma[0];
      if(distance <= max*Point() && distance > min*Point())
         {
            Print("BUY SIGNAL Distance:"+DoubleToString( distance,1));
            return true;
         }    
    }
    else if(Ask > ma[0] && directional == -1){
      double distance = ma[0]-Bid;
      if(distance <= max*Point() && distance > min*Point())
         {
            Print("SELL SIGNAL Distance:"+DoubleToString( distance,1));
            return true;
         }
    }
    if(Bid < ma[0] && directional == 1){
      double distance = ma[0] - Bid;
      if(distance <= max*Point() && distance > min*Point())
         {
            Print("SELL SIGNAL Distance:"+ DoubleToString( distance,1));
    ...
    else if(Bid < ma[0] && directional == -1){
      double distance = ma[0] + Bid;
      if(distance <= max*Point() && distance > min*Point())
         {
            Print("BUY SIGNAL Distance:"+ DoubleToString( distance,1));
...
void CalculateStops(int direction, double & tp, double  & sl)
{
   if(Tp_Type == None) tp = 0.0;
   if(Sl_Type == None) sl = 0.0;
   ...
   if(direction == 1){
      ...
      if(Tp_Type == Fix) tp=ask+TP_Point*Point();
      else if(Tp_Type == ATR){
         thresh = TP_Point * atr[0];  
         tp=ask+thresh;
      }
      if(Sl_Type ==Fix) sl = ask - SL_Value*Point();
      else if( Sl_Type == ATR){
         thresh = SL_Value * atr[0];
         sl=ask-thresh;
         if(ask - sl < min_stop*_Point){
            Print("Invalid sl. Sl Smaller than minimum stop allowed");
         }
      }
   }
   else if(direction ==-1){
      if(Tp_Type == Fix)tp=bid-TP_Point*Point();
      else if(Tp_Type == ATR){
         thresh = TP_Point * atr[0];  
         tp = bid-thresh;
      }   
      if(Sl_Type ==Fix) sl = bid + SL_Value*Point();
      if(Sl_Type == ATR){
         thresh = SL_Value * atr[0];
         sl = bid+thresh;
         if(bid + sl < min_stop*_Point){
            Print("Invalid sl. Sl Smaller than minimum stop allowed");
         }
      }
   }
