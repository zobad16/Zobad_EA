...
datetime Expiry=D'2022.08.08 00:00';
...
double Average_Open_Price(int op){
   ...
   for(int i=PositionsTotal()-1; i>=0; i--){ // returns the number of current positions
      if(m_position.SelectByIndex(i)) // selects the position by index for further access to its properties
         if(m_position.Magic()==magic_num && m_position.PositionType() == (ENUM_POSITION_TYPE)op && m_position.Comment() ==Comment_Order )
         {
            lot = m_position.Volume();
            sum_lots += lot;
            weighted_price = lot*m_position.PriceOpen();
...
void BTP_Average()
{
   ...
   for(int i=0; i<PositionsTotal(); i++)
   {
      if(PositionGetTicket(i))
      {
         if(PositionGetInteger(POSITION_MAGIC)==magic_num && PositionGetString(POSITION_SYMBOL)==Symbol() && PositionGetInteger(POSITION_TYPE)==0 && PositionGetString(POSITION_COMMENT) ==Comment_Order )
            trade.PositionModify(PositionGetTicket(i),NULL,Round2Ticksize(P_Avg+TP_Point*Point()));
...
void STP_Average()
{
   ...
   for(int i=0; i<PositionsTotal(); i++)
   {
      if(PositionGetTicket(i))
      {
         if(PositionGetInteger(POSITION_MAGIC)==magic_num && PositionGetString(POSITION_SYMBOL)==Symbol() && PositionGetInteger(POSITION_TYPE)==1 && PositionGetString(POSITION_COMMENT) ==Comment_Order)
            trade.PositionModify(PositionGetTicket(i),NULL,Round2Ticksize(P_Avg-TP_Point*Point()));
