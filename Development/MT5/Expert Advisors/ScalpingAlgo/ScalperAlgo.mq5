...
//--- protection against the return value of "zero"
   ...
   if(totalOrders == 0 && Strategy_Mode == Auto){
      ...
      if(b_signal && UseBuySignal)m_trade.Buy(b_lots,Symbol(),m_symbol.Bid(),CalculateSL(ORDER_TYPE_BUY),CalculateTP(ORDER_TYPE_BUY),OrderComment);
      if(s_signal && UseSellSignal)m_trade.Sell(s_lots,Symbol(),m_symbol.Ask(),CalculateSL(ORDER_TYPE_SELL),CalculateTP(ORDER_TYPE_SELL),OrderComment);
...
bool IsSignalHHLL(int op){
   if(!UseHigherHigh)return true;
   
   if(op == ORDER_TYPE_BUY){
      double close1 = iClose(NULL,PERIOD_CURRENT,1); double close2 = iClose(NULL,PERIOD_CURRENT,2);
...
void MannualEntry(int op ){
   ...
   */   
   ...
   else if (op == ORDER_TYPE_SELL && UseSellSignal){
