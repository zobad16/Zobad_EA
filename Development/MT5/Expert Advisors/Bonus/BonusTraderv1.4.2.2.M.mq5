...
input bool                 UsePointStop = false;
double               TP_Point=0;//TP Point
...
datetime Expiry= D'2023.01.01 00:00';
...
if(orders_total == 0){
   ...
   if(Strategy_Type==2 && (Grid_Direction==0||Grid_Direction==2) && OrdersTT(0)==0)
   {
      ...
      sl = SymbolInfoDouble(Symbol(),SYMBOL_ASK)-SL_Point*Point();     
      tp = SymbolInfoDouble(Symbol(),SYMBOL_ASK)+TP_Point*Point(); 
      if(TP_Point == 0.0)tp=0.0;
      if(SL_Point == 0.0)sl =0.0;
       
      if(UsePointStop)buy = trade.Buy(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_ASK),sl,tp,Trade_Comment);  
    ...
    {
      double sl = 0.0, tp=0.0;
      sl = SymbolInfoDouble(Symbol(),SYMBOL_BID)+SL_Point*Point();     
      tp = SymbolInfoDouble(Symbol(),SYMBOL_BID)-TP_Point*Point(); 
      if(TP_Point == 0.0)tp=0.0;
      if(SL_Point == 0.0)sl =0.0;
            ...
            bool sell = false;
            if(UsePointStop) sell = trade.Sell(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_BID),sl,tp,Trade_Comment);    
      ...
      if( order!=0)
      {
         ...
         if(Count_B==0){
            if(Grid_Direction == Grid_Both){
               bool buy = false;
               Print("###BUY-Opening first Leg");
               buy = trade.Buy(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_ASK),0,0,Trade_Comment);    
               if(buy){
                  double sl = 0.0, tp=0.0;
                  sl = SymbolInfoDouble(Symbol(),SYMBOL_ASK)-SL_Point*Point();     
                  if(TP_Point == 0.0)tp=0.0;
                  if(SL_Point == 0.0)sl =0.0;
                  if(UsePointStop)Average_SL(sl);
               }
            }
         }
         if(Count_S ==0)
         {
            if(Grid_Direction == Grid_Both)
            {
               Print("###SELL-Opening first Leg");
               
               bool sell = trade.Sell(Lot_Size,NULL,SymbolInfoDouble(Symbol(),SYMBOL_BID),0,0,Trade_Comment);
               if(sell){
                     double sl = 0.0, tp=0.0;
                     sl = SymbolInfoDouble(Symbol(),SYMBOL_BID)+SL_Point*Point() ;
                     if(TP_Point == 0.0)tp=0.0;
         ...
         if(Count_B<Grid_Max_Leg+1){
            ...
            {
               Print("##BUY LEG-Total Orders: ["+IntegerToString(order)+"]");
               ...
               //|| (Strategy_Type == MA_Directional || Strategy_Type == MA_Reversal || Strategy_Type == Mannual ))
                  ...
                  if(trade.Buy(lot_size_diff,NULL,SymbolInfoDouble(Symbol(),SYMBOL_ASK),0,0,Trade_Comment))
                  {
                     ...
                     Print("AVG["+DoubleToString(average_price_buy)+"] BUYS["+IntegerToString(Count_B)+"] SL_Point*point["+DoubleToString(SL_Point*Point())+"] point[]");
         ...
         if(Count_S<Grid_Max_Leg+1)
         {
            ...
            //|| (Strategy_Type == MA_Directional || Strategy_Type == MA_Reversal|| Strategy_Type == Mannual)){
               ...
               {
                  Print("##SELL LEG- Total Orders: ["+IntegerToString(order)+"] ");
                  ...
                  //Print("Sell grid leg["+IntegerToString(Count_S)+"] lotg["+DoubleToString(lot_size_G)+"] lotdiff["+DoubleToString(lot_size_diff)+"]");
                  ...
                  if(sell){
                     ...
                     Print("AVG["+DoubleToString(average_price_sell,Digits())+"] SELLS["+IntegerToString(Count_S)+"] SL_Point*point["+DoubleToString(SL_Point*Point())+"] point[]");
...
{
   double lot =0.0, sum_lots=0.0, price =0.0, weighted_price=0.0, sum_weighted=0.0, avg_price=0.0;
   for(int i=PositionsTotal(); i>=0; i--){ // returns the number of current positions
      if(m_position.SelectByIndex(i)) // selects the position by index for further access to its properties
         if(m_position.Magic()==magic_num && m_position.PositionType() == (ENUM_POSITION_TYPE)op && m_position.Comment() ==Trade_Comment)
         {
            lot = m_position.Volume();
            sum_lots+=lot;
            price = m_position.PriceOpen();
            weighted_price = price*lot;
            sum_weighted += weighted_price;
         }   
   }
   Print("Sum Weighted: "+DoubleToString(sum_weighted)+" |Sum lots: "+DoubleToString(sum_lots));
   avg_price = sum_weighted/sum_lots;
   string _op = "";
   if(op == POSITION_TYPE_BUY)_op = "BUY";
   else if (op == POSITION_TYPE_SELL)_op="SELL";
   Print("Weighted Average Price: OP:"+_op+ " | AVG PRICE: "+IntegerToString(avg_price));
   return avg_price;           
}
...
void MannualEntry(int op)
{
   ...
   if(UsePointStop){
      if(TP_Point ==0.0){tpb = 0.0; tps=0.0;}
      else if(TP_Point >0.0){tpb = Round2Ticksize(SymbolInfoDouble(Symbol(),SYMBOL_ASK)+TP_Point*Point()); tps = Round2Ticksize(SymbolInfoDouble(Symbol(),SYMBOL_BID)-TP_Point*Point());}
      if(SL_Point >0){slb = SymbolInfoDouble(Symbol(),SYMBOL_ASK)-SL_Point*Point();sls = Round2Ticksize(SymbolInfoDouble(Symbol(),SYMBOL_BID)+SL_Point*Point());}
      else {slb=0.0;sls=0.0;}
   }
   else if(!UsePointStop){
      tpb = 0;
      slb = 0;
      tps = 0;
      sls = 0;
   ...
   if(op == ORDER_TYPE_BUY){
      Print("BUY::TP: "+DoubleToString(tpb)+" |SL: "+DoubleToString(slb));
      ...
      Print("SELL::TP: "+DoubleToString(tps)+" |SL: "+DoubleToString(sls));
