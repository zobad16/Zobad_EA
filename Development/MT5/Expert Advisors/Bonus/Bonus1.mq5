   ...
   if(Positive_Grid)
   {
      ...
      if( order!=0)
      {
         ...
         if(Count_B<Grid_Max_Leg+1){
            double lst_price =  LastOrderPrice(ORDER_TYPE_BUY);
            if(SymbolInfoDouble(Symbol(),SYMBOL_ASK)>=(lst_price+Grid_Leg_Threshold*Point()))
            {
               Print("Leg trigger-Buy["+(Count_B+1)+"]");
               ...
               //|| (Strategy_Type == MA_Directional || Strategy_Type == MA_Reversal || Strategy_Type == Mannual ))
                  ...
                  Count_B++;
                  if(trade.Buy(lot_size_G,NULL,SymbolInfoDouble(Symbol(),SYMBOL_ASK),0,0,Trade_Comment))
                  {
                     C_B_P++;
                     GB_P[C_B_P]=SymbolInfoDouble(Symbol(),SYMBOL_ASK);
         ...
         if(Count_S<Grid_Max_Leg+1)
         {
            ...
            //|| (Strategy_Type == MA_Directional || Strategy_Type == MA_Reversal|| Strategy_Type == Mannual)){
               ...
               {
                  Print("Leg trigger-Sell["+(Count_S+1)+"]");
                  ...
                  bool sell = trade.Sell(lot_size_G,NULL,SymbolInfoDouble(Symbol(),SYMBOL_BID),0,0,Trade_Comment);
                  if(sell){
                     C_S_P++;
                     GS_P[C_S_P]=SymbolInfoDouble(Symbol(),SYMBOL_BID);
