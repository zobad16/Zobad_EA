 ...
 if(Strategy_Type==2 && (Grid_Direction==1||Grid_Direction==2) && CalculateCurrentOrders(1)==0)
 {
   ...
   if(Negative_Grid_Enable)
   {
      if(C_B!=-1 &&  CalculateCurrentOrders2( )!=0 && CalculateCurrentOrders2()<(Grid_Max_Legs))
      {
         ...
         {
            C_B++;
            double lot_size_G=NormalizeDouble(CalculateOrderLots(1 )*Grid_Multiplier,2);
...
void CheckLoss()
{
   ...
   if(num<Grid_Risk_Money*(-1))
   {
      ...
      close(0);
      close(1);
      Print("Negative Grid-"+Symbol()+"-Risk money reached -"+(string)Grid_Risk_Money+". Closing positions ...");
      Alert("Negative Grid-"+Symbol()+"-Risk money reached -"+(string)Grid_Risk_Money+". Closing positions ...");
      C_B=-1;
      C_S=-1;
   }
...
void CheckProfit()
{
   ...
   if(num>=TP_Money)
   {
      ...
      close(0);
      close(1);
      Print("Negative Grid-"+Symbol()+"-Profit money reached -"+(string)TP_Money+". Closing positions ...");
      Alert("Negative Grid-"+Symbol()+"-Profit money reached -"+(string)TP_Money+". Closing positions ...");
      C_B=-1;
      C_S=-1;
   }
