...
if(orders_total == 0){
   ...
   if(Strategy_Type==2 && (Grid_Direction==0||Grid_Direction==2) && OrdersTT(0)==0)
   {
     ...
     else if(Strategy_Type==2 && (Grid_Direction==1||Grid_Direction==2) && OrdersTT(1)==0)
    {
...
int MA_Check()
{
    if(Strategy_Type==MA_Directional || Strategy_Type == MA_Reversal){
      ...
      if(!SymbolInfoTick(_Symbol,tick)) { Print("no tick data available, error = ",GetLastError()); ExpertRemove(); return -99; }
       double Ask=tick.ask;
       double Bid=tick.bid;
      if(Ask > ma[0] && Strategy_Type==MA_Directional  )signal = 1;
      else if(Bid <ma[0]&& Strategy_Type==MA_Directional) signal = -1;
      if(Ask > ma[0] && Strategy_Type==MA_Reversal  )signal = -1;
      else if(Bid <ma[0]&& Strategy_Type==MA_Reversal) signal = 1;
      return signal;   
    }
   else{
      return 0;
