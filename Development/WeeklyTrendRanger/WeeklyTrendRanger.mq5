...
//FlagValue.Text((string)_flag);
   ...
   TimeValue.Text((string)pipDifference);
...
//Comment("ask["+ask+"]bid["+bid+"]flag["+flag+"]");
   /* if(CheckTradeTime() && AutoTimeEntry && !trade_time){
       Inputs[0].Text( ""+(string)SymbolInfoDouble(Symbol(),SYMBOL_BID));
       SetParameters();
       trade_time = true;
    }
    else
   ...
   if(!_flag && Threshold_Price > 0 && Lot_Size > 0) {
      ...
      if(AllowMonday && (dayOfWeek >0 && dayOfWeek <6)) {
         ...
         if(bid > Threshold_Price) {
            double difference = bid - Threshold_Price;
            int pipDifference = MathAbs(difference)/pricePerPips;
            Print("Checking for sell signal");
            Print(__FUNCTION__," Entry Threshold: ",Entry_Threshold,"  pip diff : ", pipDifference, "  2nd condition: ", (MathAbs(Tp_Fix * Point())/pricePerPips), " is in range: ", (pipDifference < MathAbs(Tp_Fix * Point())/pricePerPips));
...
//+------------------------------------------------------------------+
void SetParameters() {
   Threshold_Price = StringToDouble(Inputs[0].Text());
...
void PlaceOrder(int op) {
   ...
   if(op == ORDER_TYPE_BUY) {
      if(UseReversal) {
         ticket = trade.Buy(lots, Trade_Symbol, _ask, NormalizePrice(_ask-(Sl_Fix*_point)), NormalizePrice(_ask+(+Tp_Fix*_point)), TradeComment);
         _flag =true;
      } else {
         ticket = trade.Buy(lots, Trade_Symbol, _ask, NormalizePrice(_bid-(Sl_Fix*_point)), NormalizePrice(_ask+(+Tp_Fix*_point)), TradeComment);
         _flag =true;
      }
   }
      ...
      if(!UseReversal) {
         ticket = trade.Sell(lots, Trade_Symbol, _bid, NormalizePrice(_bid+(Sl_Fix*_point)), NormalizePrice(_bid-(Tp_Fix*_point)), TradeComment);
         _flag = true;
      } else {
         ticket = trade.Sell(lots, Trade_Symbol, _bid, NormalizePrice(_bid+(Sl_Fix*_point)), NormalizePrice(_bid-(Tp_Fix*_point)), TradeComment);
         _flag = true;
