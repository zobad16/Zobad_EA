...
input string            TradeComment      = "BonusPro";
...
//Not used
bool              AutoTimeEntry     = false;
bool              UseMannualEntry   = true; //Use Manual entry
bool              AllowMonday       = true; //Trade on Monday

bool             UseDirectional        = false; //Use Directional
bool             UsePartialClose       = false; //Use Partial close
double           PartialClosePoints    = 600.00;//Partial Close Points
double           PartialClosePercent   = 50.00; //Partial Close Percent
bool             UseJumptoBreakeven    = false; //Use Jump to Breakeven
double           BreakevenTriggerPoints= 600.00;//Breakeven at points
double           BreakevenPoints       = 50.00; //Points to add from breakeven
   ...
   if(id == CHARTEVENT_OBJECT_CLICK) {
      if(id == CHARTEVENT_OBJECT_CLICK) {
         ...
         if(sparam == "PBuy_Btn") {
            ...
            PlacePending(ORDER_TYPE_BUY,TradeComment);
            _flag = true;
            return;
         }
         if(sparam == "PSell_Btn") {
            ...
            PlacePending(ORDER_TYPE_SELL,TradeComment);
            _flag = true;
            return;
         }
         if(sparam == "Buy_Btn") {
...
void PlacePending(int op, string comment) {
   ...
   if(op == ORDER_TYPE_BUY) {
      //Buy limit
      if(Threshold_Price < _ask){
         ticket = trade.BuyLimit(lots, Threshold_Price,Trade_Symbol, NormalizePrice(Threshold_Price-(Sl_Fix*_point)), NormalizePrice(Threshold_Price+(+Tp_Fix*_point)),0,0, comment);
         _flag =true;         
      }
      //Buy stop
      else if(Threshold_Price > _ask){
         ticket = trade.BuyStop(lots, Threshold_Price,Trade_Symbol, NormalizePrice(Threshold_Price-(Sl_Fix*_point)), NormalizePrice(Threshold_Price+(+Tp_Fix*_point)),0,0, comment);
         _flag =true;         
      }      
   }
      ...
      //Sell limit
      if(Threshold_Price > _bid){
         ticket = trade.SellLimit(lots, Threshold_Price,Trade_Symbol, NormalizePrice(Threshold_Price+(Sl_Fix*_point)), NormalizePrice(Threshold_Price-(Tp_Fix*_point)),0,0, comment);
         _flag =true;         
      }
      //Sell stop
      else if(Threshold_Price < _bid){
         ticket = trade.SellStop(lots, Threshold_Price,Trade_Symbol, NormalizePrice(Threshold_Price+(Sl_Fix*_point)), NormalizePrice(Threshold_Price-(Tp_Fix*_point)),0,0, comment);
         _flag =true;         
      }   
   }
...
void InitializeGUI(){
   Dialog.Create(ChartID(), "                                      ", 0, 5, 20, 440, 380);
   ...
   InputLabels[0].Text("Pending Price: ");
   ...
   MannualEntryBtn[0].Create(0, "Close_Btn", 0, 6, 200, 0, 0)           ;
   MannualEntryBtn[0].Text("Close")                               ;
   MannualEntryBtn[0].Height(28)                                  ;
   ...
   MannualEntryBtn[1].Create(0, "Reset_Btn", 0, 6+110, 200, 0, 0)           ;
   MannualEntryBtn[1].Text("Reset")                               ;
   MannualEntryBtn[1].Height(28)                                  ;
   ...
   MannualEntryBtn[2].Create(0, "Ok_Btn", 0, 6, 227, 0, 0)           ;
   MannualEntryBtn[2].Text("Apply Parameters")                               ;
   MannualEntryBtn[2].Height(28)                                  ;
   MannualEntryBtn[2].Width(224)                                  ;
   MannualEntryBtn[2].Color(clrBlack)                           ;
   MannualEntryBtn[2].ColorBackground(clrLime)       ;
   MannualEntryBtn[2].ColorBorder(clrBlack)                       ;
   Dialog.Add(MannualEntryBtn[2])                                 ;

   MannualEntryBtn[3].Create(0, "Buy_Btn", 0, 6, 254, 0, 0)           ;
   MannualEntryBtn[3].Text("Buy")                               ;
   MannualEntryBtn[3].Height(28)                                  ;
   MannualEntryBtn[3].Width(114)                                  ;
   MannualEntryBtn[3].Color(clrWhite)                           ;
   MannualEntryBtn[3].ColorBackground(clrBlue)       ;
   MannualEntryBtn[3].ColorBorder(clrBlack)                       ;
   Dialog.Add(MannualEntryBtn[3])                                 ;

   MannualEntryBtn[4].Create(0, "Sell_Btn", 0, 6+110, 254, 0, 0)           ;
   MannualEntryBtn[4].Text("Sell")                               ;
   MannualEntryBtn[4].Height(28)                                  ;
   MannualEntryBtn[4].Width(114)                                  ;
   MannualEntryBtn[4].Color(clrWhite)                           ;
   MannualEntryBtn[4].ColorBackground(clrRed)       ;
   MannualEntryBtn[4].ColorBorder(clrBlack)                       ;
   Dialog.Add(MannualEntryBtn[4])                                 ;

   MannualEntryBtn[5].Create(0, "PBuy_Btn", 0, 6, 281, 0, 0)           ;
   MannualEntryBtn[5].Text("Pending Buy")                               ;
   ...
   MannualEntryBtn[6].Create(0, "PSell_Btn", 0, 6+110, 281, 0, 0)           ;
   MannualEntryBtn[6].Text("Pending Sell")                               ;
