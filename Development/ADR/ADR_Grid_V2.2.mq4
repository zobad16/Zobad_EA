...
enum GDirection
   {
      GRID_DIRECTIONAL = 1,   //Directional
      GRID_REVERSAL    = -1,  //Reversal
   };
int                    buy_counter=0;
int                    sell_counter=0;
...
Take_Profit_Type TP_Type            = VOLATILITY;             //Reversal TP:: Type
Take_Profit_Type SL_Type            = VOLATILITY;             //Reversal SL:: Type
double           TP_Value           =  4.5      ;             //Reversal TP:: Volatility/Fixed(Points)
double           SL_Value           =  1.5      ;             //Reversal SL:: Volatility/Fixed(Points)
bool             useTrail           = false     ;             //Trail:: Use Trail
_type            _trail_type1       = FIX       ;             //Trail:: Type 
double           _trailBy           = 40        ;             //Trail Volat/Fixed:: Trail by
bool             _breakEven1        = false     ;             //Breakeven:: Use jump to Breakeven
int              _whenJump1         = 25        ;             //Breakeven:: When to Jump
int              _jumpBy1           = 6         ;             //Breakeven:: Points to add after Jump

string          directionSet="-------Directional Strategy -------";             //Directional TP/SL Settings
Take_Profit_Type TP_Type1           = VOLATILITY;             //Directional TP:: Type
Take_Profit_Type SL_Type1           = VOLATILITY;             //Directional SL:: Type
double           TP_Value1          =  3.5      ;             //Directional TP:: Volatility/Fixed(Points)
double           SL_Value1          =  1.5      ;             //Directional SL:: Volatility/Fixed(Points)
bool             useTrail2          = false     ;             //Trail:: Use Trail
_type            _trail_type2       = FIX       ;             //Trail:: Type 
double           _trailBy2          = 40        ;             //Trail Volat/Fixed:: Trail by
bool             _breakEven2        = false     ;             //Breakeven:: Use jump to breakeven
int              _whenJump2         = 25        ;             //Breakeven:: When to Jump
int              _jumpBy2           = 6         ;             //Breakeven:: Points to add after Jump

extern string          gridSet="-------Grid Strategy -------";             //Grid:: Settings
bool             useGridding        = true      ;             //Grid::Use Grid
input GDirection       gDirection         = GRID_REVERSAL;          //Grid::Direction              
...
Type            legIncreaseDecrease= INCREASE  ;             //Grid::Leg Increase or decrease 
input double           increaseLLotBy     = 2         ;             //Grid::Leg Lot Increase Factor(in multiples)
double           decreaseLLotBy     = 2         ;             //Grid::Leg Lot Decrease Factor(in multiples)
...
_type            _trail_type3       = FIX       ;             //Trail:: Type 
input double           _trailBy3          = 40        ;             //Trail:: Trail by
extern double          _ep                = 200       ;             //ADR::Entry points
...
bool             _use_risk_candle1  = false     ;             //Risk Management:: Use Risk Management
int              _risk_candle1      = 4         ;             //Risk Management:: Number of Candles to read
double                 _highestStop1                  ;
double                 _lowestStop1                   ;
bool             _gapCloseCheck1    = false     ;             //Close Candle:: Use time gap
int              _whenClose1        = 50        ;             //Close Candel:: Time gap in minutes
bool                   LR_Flag            = false     ;      
double                 prev_lot           =0.0        ;
int                    _cur_total         = 0         ;
               ...
               if(isOrdersTotal(Magic_Number)>0)
               {
                       ...
                       bool useLast = true;
                        if(useGridding==true)
                        {
                           ...
                           mm.EquityBasedClose(EQ_Based,_profitTarget,useGridStop,_stopLevel,Magic_Number,ecount);
                           if(algo.OrderOperationCode(Magic_Number, BUY_LEG1) == true)
                           {
                              ...
                              op_code = GridDirection(gDirection,pointsE);
                              //Print("buyCount[",bcount,"]Main");
                              if( (op_code/*algo.Pattern_Point_Negative(pointsE, Magic_Number)*/ == REVERSAL_BUY || op_code== DIRECTIONAL_BUY )&& bcount<numberOfLegs)
                              {
                                 double nlot =0.0;
                              ...
                              else if(useGHedge==true && (op_code /*algo.Pattern_Point_Negative(pointsE, Magic_Number)*/ == REVERSAL_BUY || op_code== DIRECTIONAL_BUY )&& bcount==numberOfLegs)
                              {
                           ...
                           {}
                           else if(algo.OrderOperationCode(Magic_Number) == SELL_LEG1)
                              {
                                      // prev_lot = mm.CalculatePositionSize(Magic_Number, SELL_LEG1);
                                       ...
                                       op_code = GridDirection(gDirection,pointsE);
                                       if( (/*algo.Pattern_Point_Negative(pointsE,Magic_Number) == REVERSAL_SELL ||*/ op_code == DIRECTIONAL_SELL || op_code == REVERSAL_SELL )&& scount<numberOfLegs)
                                       {
                                          double nlot =0.0;
                                        // if(useLast == true && isOrdersTotal(Magic_Number)<_cur_total){
                                             nlot = CalculateLot2(numberOfLegs,increaseLLotBy);
                                       ...
                                       //---------------------------------------------------------------------
                                       else if( useGHedge==true &&(/*algo.Pattern_Point_Negative(pointsE, Magic_Number)*/op_code == REVERSAL_SELL ||op_code == DIRECTIONAL_SELL )&& scount==numberOfLegs)
                                       {
                                             double pre_lot,n_lot= 0.0;
                                             int count , op_c;
...
double CalculateLot2(int leg, int factor)
{
   double array1[];
   double array2[];
   ArrayResize(array1, leg,leg );
   ArrayResize(array2, leg,leg );
   double prev = startLot;
   array1[0] = prev;
   for (int ii =1 ; ii< leg; ii++){
      array1[ii] = array1[ii-1]* factor;
...
int isEntrySignal()
   {
     //Print("is Entry Signal");
     if(_strat_type == ADR_REVERSAL)
     {
      Print("ADR Reversal");
      return algo.ADR_Reversal(_ep);
     
     }
     else if(_strat_type == ADR_DIRECTIONAL)
     {
      Print("ADR Directional");
      return algo.ADR_Directional(_ep);
     }
     else if(_strat_type == DIRECTIONAL){
         int ccode = algo.Directional();
         if(ccode == DIRECTIONAL_BUY || ccode== DIRECTIONAL_SELL)return ccode;
      }
      else if(_strat_type == REVERSAL){
            int ccode = algo.Reversal();
            if(ccode == REVERSAL_BUY || ccode== REVERSAL_SELL)return ccode;
      }
      else if(_strat_type == BREAKIN){
...
int GridDirection(int direction, double epoint)
   {
      if(direction == GRID_DIRECTIONAL)
              return algo.Pattern_Point(epoint, Magic_Number);        
      else if(direction == GRID_REVERSAL)
              return algo.Pattern_Point_Negative(epoint, Magic_Number);      
      return FAIL;
