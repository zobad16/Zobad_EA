...
double CalculateTotalLotSize(string comment)
{
   double lot =0.0;
   int total = OrdersTotal();
   for(int i= total-1; i >0 ; i --){     
...
bool Clear_All_TpSL(string comment, string newcomment)
{
   int total =OrdersTotal();
   for(int i=total-1 ;i>0; i--){
      if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)){
         if(OrderMagicNumber() == Magic_Number && OrderSymbol() == Symbol()
            && OrderComment() == comment){
             bool tckt=OrderModify(OrderTicket(),OrderOpenPrice(),0,0,0,clrNONE);
             if(!tckt){
                Print("Error Reset Modify: Error No[",GetLastError(),"]");
                return false;
             }   
         }
      }
