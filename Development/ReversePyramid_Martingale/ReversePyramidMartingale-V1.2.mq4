//+------------------------------------------------------------------+
//|                                     ReversePyramidMartingale.mq4 |
//|                                                    Zobad Mahmood |
//|                                            www.quantsoftware.net |
//|Updates: - Displays on chart _count, _tp,_sl.                     |
//|         - Counter set to 1 when new order opens                  |
//|         - Added Pause before reversing order                     |
//+------------------------------------------------------------------+
...
//---
   if((TimeCurrent()>expiry)){
     ...
     Comment("Leg[",_count,"] TP[",_tp,"] SL[",_sl,"]");
  }
//+------------------------------------------------------------------+
void CloseOrder(){
   int ticket=0,op = 0;
               ...
               {   
                  if(OrderType()==OP_BUY){
                     ...
                     if(OrderOpenPrice()>OrderClosePrice()&& _count<leg ){                       
                        Print("Sleep");
                        Sleep(_timegap1*60000);//1min = 60,000
                        Print("wake and reverse");
...
int IsSignal(int strat, double range){
   ...
   else if(strat == _MID_CROSS_DIRECTIONAL)
   {
         int ccode = algo.MidCrossoverDirectional();
         if(ccode == DIRECTIONAL_BUY || ccode == DIRECTIONAL_SELL) {Print("-------\nMid Crossover Triggered Directional\n--------");return ccode;}
   }
   else if(strat == _MID_CROSS_REVERSAL)
   {
         int ccode = algo.MidCrossoverReversal();
         if(ccode == REVERSAL_BUY || ccode == REVERSAL_SELL) {Print("-------\nMid Crossover Triggered Reversal\n--------");return ccode;}
   }
      return FAIL;
}
