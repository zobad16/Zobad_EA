            ...
            if(OrderSelect(ii,SELECT_BY_POS,MODE_HISTORY))//If there is the next one
              { 
               ...
               {   
                  ...
                  Print("Value:{",value,"}");
                  //Check for legs
                  //if > legs set value to 0 and break
                  if(OrderType()==OP_BUY){
                     if(OrderOpenPrice()>OrderClosePrice())
                     {
                        if(OrderProfit()<-SL_total || MM_closed == true)
                        {
                           ...
                           {
                               if(value < leg)SendOrder(Symbol(),OP_SELL,OrderLots()*multiply,Bid,3,0,0," martingale sell ",magic,Red);
                               else {SendOrder(Symbol(),OP_SELL,lotes,Bid,3,0,0," martingale sell ",magic,Red); Print("Value:{",value,"}");}
                           }                        
                     }
                     else
                     {
                  ...
                  if(OrderType()==OP_SELL){
                     if(OrderOpenPrice()<OrderClosePrice())
                     {
                        if(OrderProfit()<-SL_total || MM_closed == true )
                        {
                            MM_closed = false;
                            closeTime= TimeCurrent();                        
                            SendOrder(Symbol(),OP_BUY,lotes,Ask,3,0,0," pyramid buy ",magic,Blue);
                        }
                        else
                        {
                              if(value < leg)SendOrder(Symbol(),OP_BUY,OrderLots()*multiply,Ask,3,0,0," pyramid buy ",magic,Blue);
                              else{SendOrder(Symbol(),OP_BUY,lotes,Ask,3,0,0," pyramid buy ",magic,Blue);Print("Value:{",value,"}");}
                        }
                    }
                     else
                     {
                        SendOrder(Symbol(),OP_SELL,lotes,Bid,3,0,0," pyramid sell ",magic,Red);
                     }
                  }
                  break;                  
               }
