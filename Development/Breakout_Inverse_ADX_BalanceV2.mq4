...
input double           TP_Volatility_Factor=2.0;         //TP::Volatility Factor
input double           SL_Volatility_Factor=1.0;         //SL::Volatility Factor
input double           TP_Fixed=25.0;                    //TP::Fixed(Points)
input double           SL_Fixed=12.0;                    //SL::Fixed(Points)
input int              _timegap1=31;                     //Time Gap::Order 1 time gap(in mins)
...
double                 _lowestStop1;
input bool             _gapCloseCheck1=false;            //Risk Management::Use Close Order Time Gap
input int              _whenClose1= 50;                  //Risk Management::Time Gap (Minutes)
       

extern string          order1v2="-------Order_1V2--------";//Order 1v2 Settings
bool                   order1v2Open=false;
extern string          Strat_Name1v2="Break Out2 Inverse"; //Strategy Name
input bool             useStrategy1v2=false;               //Order 1v2::Use Strategy
input double           LotSize1v2=1.0;                     //Order 1v2::Lot Size
input Take_Profit_Type TP_Type1v2= VOLATILITY;             //TP::Type
input Take_Profit_Type SL_Type1v2= VOLATILITY;             //SL::Type
input double           TP_Volatility_Factor1v2=2.0;        //TP::Volatility Factor
input double           SL_Volatility_Factor1v2=1.0;        //SL::Volatility Factor
input double           TP_Fixed1v2=25.0;                   //TP::Fixed(Points)
input double           SL_Fixed1v2=12.0;                   //SL::Fixed(Points)
input int              _timegap1v2=31;                     //Time Gap::Order 1v2 time gap(in mins)
...
input bool             _gapCloseCheck1v2=true;             //Risk Management::Use Close Order Time Gap
input int              _whenClose1v2= 50;                  //Risk Management::Close Order Time Gap in minutes 
...
bool isTrend()
{
  ...
  double adx = iADX(Symbol(),adx_Chart_Timeframe,adx_Period,adx_App_Price,0,0);
  if(adx >= adx_Entry_Range)
   {
      adx = NormalizeDouble(adx, digit);
      //Print("ADX Value[",adx,"]");
      return true;
   } 
  return false;    
...
bool Buy_Order(double lot, string comment, int sl_type, double _slf,double _slv, int tp_type, double _tpf, double _tpv,int order_type, double & _atr)
{
   ...
   _atr = NormalizeDouble(atr,Digits);
...
bool Place_Buy_Order4()
{
   ...
   double min_SL= MarketInfo(Symbol(),MODE_STOPLEVEL);
   double minSL= min_SL*point;
...
bool Sell_Order(double lot, string comment, int sl_type, double _slf,double _slv, int tp_type, double _tpf, double _tpv, int order_type, double &_atr)
{
   double point= MarketInfo(Symbol(), MODE_POINT);
   int digit=(int) MarketInfo(Symbol(), MODE_DIGITS);
...
double LotMoneyManagement(double risk,double stop)
{
...
bool isConsequtiveLoss(int & losses, string name)
{
   int total =OrdersHistoryTotal();
   ...
   for(int i= total-1; i>0; i--)
   {
     if( OrderSelect(i,SELECT_BY_POS,MODE_HISTORY)>0)
     {
         if(OrderSymbol() == Symbol() && OrderMagicNumber() == Magic_Number)
         {
            if(StringFind(OrderComment(), name,0)!= -1)
            {
               if(OrderProfit()<0)
               {
                  loss++; losses++;
...
double lossLotmanagement(double prevl, double percent)
{
   double  lotStep = MarketInfo(Symbol(), MODE_LOTSTEP);
   double  minLot  = MarketInfo(Symbol(), MODE_MINLOT);
   double  tickVal = MarketInfo(Symbol(), MODE_TICKVALUE);
   percent=percent/100;
   double lot = prevl * percent;
   lot = prevl-lot;
   lot = MathRound(lot/lotStep)*lotStep;
   if(lot < minLot)
    {
...
//---
   ...
   if(isTrend())
   {
      if(order1Open)
         {
            if(_trail_type1 == VOLAT)
            {
               Trail_Volatility(prevAtr,_trailVolat_1);
            }
            else if(_trail_type1 == FIX)
            {
      ...
      if(IsNewBar())
         {
           ...
           // Print("Number of Consequtive Losses[",lossTrades,"]");
            ...
            {
                if(lossTrades>noConsqLossAllowed && useConsecutive)
                {
                  lot = lossLotmanagement(lot,percentReduction);
                }
                Sell_Order(lot,Strat_Name,SL_Type,SL_Fixed,SL_Volatility_Factor,
                          TP_Type,TP_Fixed,TP_Volatility_Factor,OP_SELL,atr);
                prevAtr = atr;
            ...
            if(Pattern1()==SELL && !buyIgnore1==true)
               {
                  if(lossTrades>noConsqLossAllowed && useConsecutive)
                {
                  ...
                  Buy_Order(lot,Strat_Name,SL_Type,SL_Fixed,SL_Volatility_Factor,
                             TP_Type,TP_Fixed,TP_Volatility_Factor,OP_BUY,atr);
           ...
           if(useStrategy1v2)
           {
            ...
            double lot =LotSize1v2;  
            if(Pattern1v2()==BUY && !buyIgnore1v2==true)
            {
                Sell_Order(lot,Strat_Name1v2,SL_Type1v2,SL_Fixed1v2,SL_Volatility_Factor1v2,
                           TP_Type1v2,TP_Fixed1v2,TP_Volatility_Factor1v2,OP_SELL,atr);
            ...
            if(Pattern1v2()==SELL && !buyIgnore1v2==true)
               {
                  Buy_Order(lot,Strat_Name1v2,SL_Type1v2,SL_Fixed1v2,SL_Volatility_Factor1v2,
                             TP_Type1v2,TP_Fixed1v2,TP_Volatility_Factor1v2,OP_BUY,atr);
           ...
           if(useStrategy2)
           {
            ...
            if(Pattern2()==BUY && sellIgnore2==false)
               {
                   Sell_Order(lot,Strat_Name2,SL_Type2,SL_Fixed2,SL_Volatility_Factor2,
                             TP_Type2,TP_Fixed2,TP_Volatility_Factor2,OP_SELL,atr);
               ...
               {
                  Buy_Order(lot,Strat_Name2,SL_Type2,SL_Fixed2,SL_Volatility_Factor2,
                             TP_Type2,TP_Fixed2,TP_Volatility_Factor2,OP_BUY,atr);
          ...
          if(useStrategy3 )
           {
            ...
            if(Pattern3()==BUY && sellIgnore3==false)
               { 
                  Sell_Order(lot,Strat_Name3,SL_Type3,SL_Fixed3,SL_Volatility_Factor3,
                             TP_Type3,TP_Fixed3,TP_Volatility_Factor3,OP_SELL,atr);
               ...
               {
                  Buy_Order(lot,Strat_Name3,SL_Type3,SL_Fixed3,SL_Volatility_Factor3,
                             TP_Type3,TP_Fixed3,TP_Volatility_Factor3,OP_BUY,atr);
