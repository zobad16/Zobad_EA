...
bool Buy_Alert()
{ double bbl= iBands(NULL,BB_Chart_Timeframe,BB_Period,2,0,PRICE_CLOSE,MODE_LOWER,0);
   //Print("BBL; ",bbl);
   if(!LBandTouch)
   {
      //If bar touches or crosses the lower band
     
      if(Low[0]<=bbl)
      {
         LBandTouch=true;
         Print("Low Band Signal Activated");
         //If the previous candles pattern breaks and the closing is above the lowerband
         if(Pattern_Brake() && Close[0]> bbl && bbl< Close[0])
         //if(Pattern_Brake() && Close[0]> iBands(NULL,0,BB_Period,2,0,PRICE_CLOSE,MODE_LOWER,0))
         {
            Print("Low band Buy");
            LBandTouch=false;
            return true;
      ...
      //If comming from top and touches MidBand Sell alert
      else if(Low[0]<=iBands(NULL,BB_Chart_Timeframe,BB_Period,2,0,PRICE_CLOSE,MODE_MAIN,0))
      {
         LBandTouch=false;
         Print("Mid Band Signal Activated");
         double BB_Down=iBands(NULL,BB_Chart_Timeframe,BB_Period,2,0,PRICE_CLOSE,MODE_LOWER,0);
         double BB_Mid=iBands(NULL,BB_Chart_Timeframe,BB_Period,2,0,PRICE_CLOSE,MODE_MAIN,0);
   ...
   {
      //If the previous candles pattern breaks and the closing is bellow the upperband
         if(Close[0]> bbl&&Pattern_Brake() )
         //if(Pattern_Brake() && Close[0]> iBands(NULL,0,BB_Period,2,0,PRICE_CLOSE,MODE_LOWER,0))
         {
         Print("bbl: ",bbl);
            Print("Buy Alert. Broken Pattern below");
            Print("Low band Buy");
            LBandTouch=false;
            return true;
         }
       else if(Close[0]< iBands(NULL,0,BB_Period,2,0,PRICE_CLOSE,MODE_LOWER,0)) 
        return false;
   }
...
bool Place_Buy_Order()
{
  ...
  // Print("Ask Price: ",Ask);
   double atr=iATR(Symbol(),ATR_Charts_Period,ATR_Period,0);   Print("ATR Reading: ",atr);
